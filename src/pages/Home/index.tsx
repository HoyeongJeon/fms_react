...
import { ChatMessage, ChatWrapper, MyChatMessage } from 'components/ChatList/styles';
...
import { ErrorContainer, ErrorMessage, MyTime, OthersTime, StickyHeader } from './styles';
...
const Home = () => {
    ...
    const getKey = (pageIndex: number, previousPageData: any) => {
        ...
        return `/chats/${chatId}/messages?order__createdAt=DESC&page=${pageIndex + 1}`;
    };

    const { data: chatData, mutate: mutateChat, setSize, isValidating } = useSWRInfinite(getKey, fetcher);
    const isEmpty = chatData?.[chatData?.length - 1].data?.length === 0;
    const isReachingEnd = isEmpty || (chatData && chatData[chatData.length - 1].length < 30);
        ...
        (e: React.UIEvent<HTMLElement>) => {
            ...
            if (scrollable.scrollTop === 0 && !isReachingEnd && !isValidating) {
                ...
                setMessages((currentMessages) => {
                    ...
                    const newMessages = formattedData.filter((msg: Message) => !currentMessageIds.has(msg.id));
            ...
            axios
                ...
                .then((res: any) => {
                    ...
                    setMessages((currentMessages) => {
                        ...
                        const newMessages = formattedData.filter((msg) => !messageIds.has(msg.id));
                        return [...newMessages];
                    });
                })
                .catch((err: any) => {
    ...
    useEffect(() => {
        if (teamWinningRate?.wins && teamWinningRate?.totalGames) {
            setRate(Math.floor((teamWinningRate?.wins / teamWinningRate?.totalGames) * 100));
        }
    }, [teamWinningRate]);
