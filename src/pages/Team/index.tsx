...
const Team = () => {
  ...
  const [topGoalsPlayer, setTopGoalsPlayer] = useState<string>(
    "/img/empty_profile_iamge.png"
  );
  const [topGamesPlayer, setTopGamesPlayer] = useState<string>(
    "/img/empty_profile_iamge.png"
  );
  const [topAssistsPlayer, setTopAssistsPlayer] = useState<string>(
    "/img/empty_profile_iamge.png"
  );
  const [topAttactPointPlayer, setAttactPointPlayer] = useState<string>(
    "/img/empty_profile_iamge.png"
  );
  const [topSavePlayer, setTopSavePlayer] = useState<string>(
    "/img/empty_profile_iamge.png"
  );
  const [yellowAndRedCards, setYellowAndRedCards] =
    useState<YellowAndRedCardsType>();
  ...
  useEffect(() => {
    ...
    const getYellowAndRedCards = async () => {
      const getYellowAndRedCardsAxiosData =
        await axios.get<YellowAndRedCardsType>(
          `${process.env.REACT_APP_SERVER_HOST}:${
            process.env.REACT_APP_SERVER_PORT || 3000
      ...
      <ScoreboardContainer>
        ...
        <Card className="card-div">
          ...
          <div className="team-info-graph">
            ...
            yellowAndRedCards?.yellowAndRedCards.length > 0 ? (
              <BasicBars
                yellowAndRedCards={yellowAndRedCards?.yellowAndRedCards ?? []}
              />
            ) : (
        ...
        <Card className="card-div">
          {players && <PlaneTable data={players} />}
        </Card>
      </ScoreboardContainer>
    </Layout>
  );
